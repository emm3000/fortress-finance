...
- [ ] Inicializar proyecto Node.js (`npm init -y`) y configurar `tsconfig.json` con `strict: true`.
- [ ] Instalar dependencias core de producción y desarrollo definidas arriba.
- [ ] Configurar Express en `src/app.ts` y el puerto en `src/server.ts`.
- [ ] Configurar ESLint y Prettier. Asegurarse que están sincronizados (con eslint-config-prettier).
- [ ] Configurar Git Hooks con `husky` y `lint-staged` en el `package.json`.
- [ ] Configurar `vitest` para las pruebas ejecutando scripts (`npm run test`).
